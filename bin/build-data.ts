...
import { type JourneySchema, type QuestSchema, convertJourneyCanvas } from '../lib/core/core';
import { convertQuestMarkdown } from '../lib/core/convert-quest-markdown';
import { isJourneyCanvasAvailable, findQuestMarkdown } from '../lib/core/extract-content';
import { type SoloQuestSchema, type SoloQuestShortSchema } from '../lib/core/schemas';
...
export function buildJourneyDataFiles(journeyPath: string, outputDir: string = 'output'): void {
  ...
  writeFileSync(path.join(outputDir, journeyFileName), JSON.stringify(journey, null, 2));
  ...
  quests.forEach((quest) => {
    ...
    writeFileSync(path.join(questDirPath, questFileName), JSON.stringify(quest, null, 2));
  });
}

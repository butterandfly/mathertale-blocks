...
export class ProofReorderBlockData implements BlockSchema {
  id: string;
  ...
  constructor(id: string, content: string, questionData: ProofReorderQuestionData, name?: string) {
    this.id = id;
  ...
  getText(): string {
    const { orderItems } = this.questionData;
    let text = 'Proof:\n\n' + this.content + '\n\n';
  ...
  static validate(block: ProofReorderBlockData): void {
   ...
   *
   */
  static fromMarkdown(markdown: MarkdownBlock): ProofReorderBlockData {
    ...
    const newBlock = new ProofReorderBlockData(
      markdown.id,
      content,
      {
...
export function convertProofReorderMarkdown(markdown: MarkdownBlock): ProofReorderBlockData {
  return ProofReorderBlockData.fromMarkdown(markdown);
}

...
export class ContradictionBlockData implements BlockSchema {
  id: string;
  ...
  constructor(id: string, content: string, questionData: ContradictionQuestionData, name?: string) {
    this.id = id;
  ...
  static validate(block: ContradictionBlockData): void {
    ...
    if (!explanation) {
      // Assuming explanation is always required based on fromNode/checkRequiredProperties
   ...
   * ```
   */
  static fromMarkdown(markdown: MarkdownBlock): ContradictionBlockData {
    ...
    const block = new ContradictionBlockData(
      markdown.id,
...
export function convertContradictionMarkdown(markdown: MarkdownBlock): ContradictionBlockData {
  return ContradictionBlockData.fromMarkdown(markdown);
}
